...
const createPosition = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(409)
      .json(error("This position already exists", res.statusCode));
  } catch (err) {
...
const getPositionsByRoleId = async (req, res) => {
  ...
  try {
    const position = await positionModel
      .find({ roleId: req.params.id })
      .select("-roleId -code");
    if (!position) {
      return res
        .status(409)
        .json(error("This position doesn't exist", res.statusCode));
    }
    return res
      .status(200)
...
const updatePosition = async (req, res) => {
  ...
  try {
    ...
    if (!existingId) {
      return res
        .status(409)
        .json(error("This position Id doesn't exist", res.statusCode));
    }
    ...
    if (existingPosition) {
      return res
        ...
        .json(
          error(
            "You have already created this position name. Please try a different one",
            res.statusCode
          )
        );
    } else {
